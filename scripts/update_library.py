...
def rank(pool, seen, today):
    remaining = [dict(p) for p in pool if classify(p['venue']) and not keys(p) & seen]
    ranked = []; used = set()
    while remaining:
        choices = [p for p in remaining if (p['group'], p['topic']) not in used]
        if not choices:
            used.clear(); choices = remaining
        p = max(choices, key=lambda p: (score(p, today), p['title']))
        remaining.remove(p); ranked.append(p); used.add((p['group'], p['topic']))
    return ranked

def make_additions(base, candidates, today, cache, max_attempts=6):
    ...
    for candidate in candidates[:24]:
        ...
        if keys(p) & seen:
            continue
        entry = {'title': p['title']}; audit.append(entry)
        try:
            ...
            p.pop('_abstract', None); p.pop('citations', None); p['id'] = uid(p)
...
def run(dry_run=False, output=None, discover_only=False):
    ...
    pool, report = collect(today); candidates = rank(pool, seen, today)
    ...
    if candidates:
        with local_model() as base:
            additions, audit = make_additions(base, candidates, today, cache)
    ...
    save_json(LIBRARY, result); save_json(STATUS, status)
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(); run(args.dry_run, args.output, args.discover_only)
