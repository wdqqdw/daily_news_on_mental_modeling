...
class Document(HTMLParser):
    ...
    def handle_starttag(self,tag,attrs):
        ...
        if tag=='article' and a.get('class')=='paper-card':self.papers+=1
...
def check():
    ...
    for path in SITE.rglob('*.html'):
        ...
        if path.name!='archive.html':assert doc.papers==3
    ...
    print(f'Validated {len(docs)} pages, 3 papers/issue, all links and anchors')
