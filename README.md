...
- **每天同时找新文和旧文，不再有两年限制。** 近期通道检索过去 365 天；历史通道从 arXiv 早期记录和 Crossref 1900 年起的记录回溯到一年前，并轮换旧年份 ACL 会议。arXiv 每天检索两组近期关键词和三个历史主题；历史检索按主题、刊物轮换与分页，检索位置保存在 `data/collection_state.json`。
...
- 近期与历史候选交替进入筛选，再兼顾研究方向和来源多样性。每轮最多审阅 24 个候选、尝试生成 8 篇研究笔记，以控制运行时间；**没有最低篇数要求**。不通过的论文 90 天内不反复审阅，处理错误三天后可重试。单篇失败不丢弃已经核验的条目；模型、全部来源或所有 arXiv 检索失败时保留线上版本并让工作流报错。
