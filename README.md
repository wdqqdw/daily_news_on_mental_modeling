...
仅用情绪词或“心理健康”描述应用价值的有害内容分类，也不纳入。内容安全相关论文只有明确研究心智状态、信念、意图或认知评价机制时才保留候选资格。
...
2. 以标题中的明确心智对象、摘要中的计算方法及实验内容初筛。摘要必须在贡献声明中提出模型、方法、框架或算法；只提出基准或数据集、再与已有模型比较的论文不通过。缺少可用摘要不生成解读。
